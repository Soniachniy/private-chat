...
const chatDropdownItems = [
	{ title: 'Pin', icon: <Bookmark stroke="white" />, type: DropdownType.Item },
	{ title: 'Rename', icon: <Pencil stroke="white" />, type: DropdownType.Item },
	{ title: 'Clone', icon: <Clone stroke="white" />, type: DropdownType.Item },
	{ title: 'Archive', icon: <Archive stroke="white" />, type: DropdownType.Item },
	{ title: 'Download', icon: <Download stroke="white" />, type: DropdownType.Item },
	{ title: 'Delete', icon: <Trash stroke="white" />, type: DropdownType.Item }
];
											...
											<div className="w-full  relative group" key={chat.id} draggable="true">
												<a
													className={
														...
														(chat.id === currentChatId ? ' bg-[#00ec9714]' : '')
													}
													href={`/c/${chat.id}`}
													draggable="false"
												>
							...
							>
								{dropdownItems.map((el) => {
									if (el.type === DropdownType.Separator) {
										return <DropdownMenuSeparator className="border-gray-100 bg-gray-850" />;
									} else {
										return (
											<DropdownMenuItem className="flex flex-row gap-2 py-2 px-3 hover:bg-gray-800 focus:bg-gray-800 text-white">
												{el.icon} {el.title}
											</DropdownMenuItem>
										);
									}
